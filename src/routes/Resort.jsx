...
import RESORT_DATA from "../components/Resort/RESORT_DATA";
import Footer from "../components/Footer/Footer";
const Resort = () => {
  const { rid } = useParams();
  const resortDetailData = RESORT_DATA.filter((data) => data.slug === rid);

  return (
    <Fragment>
      <div>
        {resortDetailData.map((resort) => (
          <div key={resort.id}>
            ...
            <Container>
              <h1 className="font-semibold text-3xl my-6">{resort.heading}</h1>
              <div className="grid grid-cols-4 gap-10">
                {resort.items.map((item, index) => (
                  <div
                    key={index}
